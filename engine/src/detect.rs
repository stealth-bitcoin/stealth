...
impl TxGraph {
    ...
    fn detect_address_reuse(&self, findings: &mut Vec<Finding>) {
        for addr in self.our_addrs.clone() {
            ...
            if receive_txids.len() >= 2 {
                ...
                let role = if meta.is_some_and(|m| m.internal) {
                    "change"
                } else {
                    "receive"
                };
    ...
    fn detect_behavioral_fingerprint(&self, findings: &mut Vec<Finding>) {
        ...
        let txids: Vec<String> = self.our_txids.iter().cloned().collect();
        let send_labeled_txids: HashSet<String> = self
            .addr_txs
            .values()
            .flatten()
            .filter(|entry| entry.category == "send" && !entry.txid.is_empty())
    ...
    fn detect_dust_attack(&self, findings: &mut Vec<Finding>) {
        ...
        for txid in &txids {
            ...
            let unique_addrs: HashSet<&str> = outputs
                .iter()
                .filter(|o| !o.address.is_empty())
                .map(|o| o.address.as_str())
                .collect();
            let diversity = unique_addrs.len() as f64 / outputs.len().max(1) as f64;
            if diversity < 0.8 {
                continue;
            }
    ...
    fn detect_peel_chain(&self, findings: &mut Vec<Finding>) {
        ...
        for txid in &txids {
            ...
            while hops < max_hops {
                ...
                let child_txid = self.find_spending_tx(&trace_txid, trace_vout as u32);
    ...
    fn detect_deterministic_links(
        ...
        for txid in &txids {
            ...
            for i in 0..n_in {
                for j in 0..n_out {
                    if pair_count[i][j] == total_valid {
                        det_links.push(json!({
                            "input_index": i,
                            "output_index": j,
                            "input_address": inputs[i].address,
                            "output_address": outputs[j].address,
    ...
    fn detect_toxic_change(&self, findings: &mut Vec<Finding>) {
        ...
        for txid in &txids {
            ...
            for out in &outputs {
                ...
                let child_txid = self.find_spending_tx(txid, out.index as u32);
                let child_txid = match child_txid {
                    Some(t) => t,
                    None => continue,
                };
